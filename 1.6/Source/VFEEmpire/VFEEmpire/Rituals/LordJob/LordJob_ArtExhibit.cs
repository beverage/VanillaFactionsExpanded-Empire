...
public class LordJob_ArtExhibit : LordJob_Ritual
{
    ...
    private readonly Dictionary<Thing, CellRect> cachedSpectate = new(); //caching these as constant checking might be a bit much
    ...
    public Room Gallery
    {
        get
        {
            if (gallery == null) return gallery = Spot.GetRoom(lord.lordManager.map);
            return gallery;
        }
    }
    ...
    {
        if (cachedSpectate.TryGetValue(artPiece, out var rect)) return rect;
        var standable = 0f;
        foreach (var cardinal in GenAdj.CardinalDirections) //goal is to find the most open area to spectate from
        {
            var center = artPiece.Position + cardinal * 3;
            if (!center.Standable(Map) || center.GetRoom(Map) != Gallery)
                center = CellFinder.RandomClosewalkCellNear(artPiece.Position + cardinal, Map, 2,
                    c => { return c.Standable(Map) && c.GetRoom(Map) == Gallery; });
            var tmpStand = 0f;
    ...
    public override StateGraph CreateGraph()
    {
        ...
        transition_Arrived.AddTrigger(new Trigger_Custom(signal => signal.type == TriggerSignalType.Tick && leadNoble.Position == Spot));
        ...
        transition_Leave.AddTrigger(new Trigger_TickCondition(
            () => { return !GenHostility.AnyHostileActiveThreatTo(Map, Faction.OfEmpire) || Gallery.PsychologicallyOutdoors || shuttle.Destroyed; }, 60));
    ...
    public void StopExhibit(string signal)
    {
        ...
        foreach (var keyValuePair in artToil.Data.presentForTicks)
